    ...
    return (
      <div className="p-6">
        <div className="max-w-2xl mx-auto mb-4">
          <button onClick={() => setView("history")} className="text-caption text-accent-gold hover:text-accent-gold/80 transition-colors">
            ← Back to History
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden">
        {/* Left column — transcript primary panel */}
                    ...
                    {[...state.integrityViolations].reverse().map((v: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-radius-card bg-surface-raised border border-border-default">
                        ...
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-radius-card ${
                          v.severity === "severe" ? "bg-status-alert/20 text-status-alert"
